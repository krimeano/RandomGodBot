...
# -------------------------------------- # submit # -------------------------------------- #
@bot.message_handler(
    func=lambda message: True and middleware.check_post(message.chat.id) != None and message.text == language_check(message.chat.id)[1]['draw']['draw_buttons'][-2])
def submit(message):
...
def enter_photo(message):
    ...
    fsm.set_state(message.chat.id, "enter_winners_count", chanel_id=tmp['chanel_id'], chanel_name=tmp['chanel_name'], draw_text=tmp['draw_text'], file_type=file_type,
                  file_id=file_id)
    bot.send_message(message.chat.id, text['winners_count'], reply_markup=back_button)


# -------------------------------------- # enter_winners_count # -------------------------------------- #
@bot.message_handler(func=lambda message: True and fsm.get_state(message.chat.id)[0] == 'enter_winners_count')
...
def change_winners_count(message):
    ...
    bot.send_message(message.chat.id, text['winners_count'], reply_markup=keyboard.back_button(message.chat.id))


@bot.message_handler(func=lambda message: True and fsm.get_state(message.chat.id)[0] == 'change_winners_count')
